...
{
    {SPECIES_RHYHORN, 26, ITEM_NONE, {MOVE_NONE, MOVE_NONE, MOVE_NONE, MOVE_NONE}},
    {SPECIES_KANGASKHAN, 30, ITEM_NONE, {MOVE_NONE, MOVE_NONE, MOVE_NONE, MOVE_NONE}},
};

static const struct GiovanniPartyTemplateMon sGiovanniMemoryParty_Chapter2[] =
{
    {SPECIES_KANGASKHAN, 38, ITEM_NONE, {MOVE_NONE, MOVE_NONE, MOVE_NONE, MOVE_NONE}},
    {SPECIES_RHYHORN, 39, ITEM_NONE, {MOVE_NONE, MOVE_NONE, MOVE_NONE, MOVE_NONE}},
    {SPECIES_NIDOQUEEN, 42, ITEM_NONE, {MOVE_NONE, MOVE_NONE, MOVE_NONE, MOVE_NONE}},
};

static const struct GiovanniPartyTemplateMon sGiovanniMemoryParty_Chapter3[] =
{
    {SPECIES_DUGTRIO, 44, ITEM_NONE, {MOVE_EARTHQUAKE, MOVE_SLASH, MOVE_SAND_TOMB, MOVE_MUD_SLAP}},
    {SPECIES_NIDOQUEEN, 45, ITEM_NONE, {MOVE_EARTHQUAKE, MOVE_BODY_SLAM, MOVE_DOUBLE_KICK, MOVE_ROAR}},
    {SPECIES_NIDOKING, 46, ITEM_NONE, {MOVE_EARTHQUAKE, MOVE_THRASH, MOVE_DOUBLE_KICK, MOVE_ROCK_TOMB}},
    {SPECIES_RHYDON, 48, ITEM_NONE, {MOVE_EARTHQUAKE, MOVE_ROCK_BLAST, MOVE_MEGAHORN, MOVE_SCARY_FACE}},
};
...
{
    {
        ...
        .safeReturnMap = MAP_DIGLETTS_CAVE_NORTH_ENTRANCE,
        .safeReturnX = 3,
        .safeReturnY = 4,
        .recoveryFallbackX = 21,
        .recoveryFallbackY = 8,
    },
    {
        .map = MAP_ROUTE2,
    ...
    },
    {
        ...
        .npcSetId = GIOVANNI_NPC_SET_NONE,
        .guaranteedProgressWarpId = 0,
        .safeReturnMap = MAP_DIGLETTS_CAVE_SOUTH_ENTRANCE,
        .safeReturnX = 82,
        .safeReturnY = 70,
        .recoveryFallbackX = 6,
        .recoveryFallbackY = 8,
    },
    {
        ...
        .enabledTriggerMask = 0,
        .npcSetId = GIOVANNI_NPC_SET_NONE,
        .guaranteedProgressWarpId = 9,
        .safeReturnMap = MAP_ROUTE11,
        .safeReturnX = 6,
        .safeReturnY = 8,
        .recoveryFallbackX = 14,
        .recoveryFallbackY = 24,
    },
    {
        ...
        .npcSetId = GIOVANNI_NPC_SET_NONE,
        .guaranteedProgressWarpId = 0,
        .safeReturnMap = MAP_CELADON_CITY_GAME_CORNER,
        .safeReturnX = 10,
        .safeReturnY = 11,
        .recoveryFallbackX = 34,
        .recoveryFallbackY = 22,
    },
    {
        .map = MAP_CELADON_CITY_GAME_CORNER,
        .chapterId = 2,
    ...
    {
        .map = MAP_ROCKET_HIDEOUT_ELEVATOR,
        .chapterId = 2,
        .allowedWarpMask = (1 << 0) | (1 << 1),
        .enabledTriggerMask = 0,
        .npcSetId = GIOVANNI_NPC_SET_NONE,
    ...
    },
    {
        ...
        .npcSetId = GIOVANNI_NPC_SET_NONE,
        .guaranteedProgressWarpId = 0,
        .safeReturnMap = MAP_SAFFRON_CITY,
        .safeReturnX = 34,
        .safeReturnY = 29,
        .recoveryFallbackX = 6,
    ...
    },
    {
        ...
        .npcSetId = GIOVANNI_NPC_SET_NONE,
        .guaranteedProgressWarpId = 0,
        .safeReturnMap = MAP_UNDERGROUND_PATH_NORTH_SOUTH_TUNNEL,
        .safeReturnX = 4,
        .safeReturnY = 4,
        .recoveryFallbackX = 33,
        .recoveryFallbackY = 29,
    },
    {
...
static const struct GiovanniMapOverlay *GetGiovanniOverlayForMap(u16 map, u8 chapterId)
{
    u8 i;
    const struct GiovanniMapOverlay *fallback = NULL;

    for (i = 0; i < ARRAY_COUNT(sGiovanniMapOverlays); i++)
    {
        if (sGiovanniMapOverlays[i].map == map)
        {
            if (sGiovanniMapOverlays[i].chapterId == chapterId)
                return &sGiovanniMapOverlays[i];

            if (fallback == NULL)
                fallback = &sGiovanniMapOverlays[i];
        }
    }
...
static void SaveGiovanniCheckpointPositionAndStage(u8 chapterId, bool8 saveAllowedAtCheckpoint);
static void SaveGiovanniAct1StartCheckpoint(u8 chapterId, bool8 saveAllowedAtCheckpoint);
...
static u16 GetGiovanniCheckpointObjectiveFlags(void);
...
static void SetGiovanniCampaignSegment(u16 segment)
{
    ...
    UpdateGiovanniMemorySaveStateForSegment(previousSegment, segment, TRUE);
}
...
static void UpdateGiovanniMemorySaveStateForSegment(u16 previousSegment, u16 nextSegment, bool8 emitMessage)
{
    struct GiovanniMemoryModeSnapshot *snapshot;
    bool8 wasBlocked;
    bool8 nowBlocked;
    u8 chapterId;
    ...
    snapshot = GetGiovanniMemoryModeSnapshot();
    wasBlocked = !snapshot->checkpointSaveAllowed;
    nowBlocked = IsGiovanniCampaignSegmentActActive(nextSegment);

    if (wasBlocked == nowBlocked)
        return;

    snapshot->checkpointSaveAllowed = !nowBlocked;
    chapterId = GetGiovanniMemoryModeChapterId();

    if (chapterId >= 1 && chapterId <= 3)
    {
        if (nowBlocked && !IsGiovanniCampaignSegmentActActive(previousSegment))
            SaveGiovanniCheckpointPositionAndStage(chapterId, FALSE);
        else if (!nowBlocked && IsGiovanniCampaignSegmentActActive(previousSegment))
            SaveGiovanniCheckpointPositionAndStage(chapterId, TRUE);
    }

    if (emitMessage)
    {
        StringCopy(gStringVar1, nowBlocked ? gText_GiovanniMemorySaveBlocked : gText_GiovanniMemorySaveAvailable);
        StringExpandPlaceholders(gStringVar4, gText_GiovanniMemorySaveStateChanged);
...
static bool8 IsGiovanniBetweenActsSaveAllowed(void)
{
    ...
    return !IsGiovanniCampaignSegmentActActive(VarGet(VAR_GIO_SEGMENT));
}
...
bool8 IsGiovanniMemorySaveFramingAllowed(void)
{
    return FlagGet(FLAG_SYS_GIOVANNI_MEMORY_MODE_ACTIVE)
        && IsGiovanniBetweenActsSaveAllowed();
}
...
static u8 GetSanitizedGiovanniCheckpointId(u8 chapterId, u8 checkpointId)
{
    ...
    {
    case 1:
        return min(checkpointId, 1);
    case 2:
        return 0;
    case 3:
        return min(checkpointId, 3);
    default:
        return 0;
    }
}
...
static u16 GetGiovanniCheckpointObjectiveFlags(void)
{
    u16 objectiveFlags = 0;

    if (FlagGet(FLAG_ROCKETOPS_CHAPTER_OBJECTIVE_CLEARED))
        objectiveFlags |= GIO_CHECKPOINT_OBJECTIVE_CHAPTER_CLEARED;
    if (FlagGet(FLAG_GIO_MEM_CH3_ESCORT_SEGMENT_ACTIVE))
        objectiveFlags |= GIO_CHECKPOINT_OBJECTIVE_ESCORT_ACTIVE;
    if (FlagGet(FLAG_GIO_MEM_CH3_ESCORT_CHECKPOINT_1))
        objectiveFlags |= GIO_CHECKPOINT_OBJECTIVE_ESCORT_CP1;
    if (FlagGet(FLAG_GIO_MEM_CH3_ESCORT_CHECKPOINT_2))
        objectiveFlags |= GIO_CHECKPOINT_OBJECTIVE_ESCORT_CP2;
    if (FlagGet(FLAG_GIO_MEM_CH3_ACT1_BRIEFING_COMPLETE))
...
static void SnapshotGiovanniActCheckpointState(bool8 saveAllowedAtCheckpoint)
{
    struct GiovanniMemoryModeSnapshot *snapshot = GetGiovanniMemoryModeSnapshot();

    snapshot->checkpointChapter = VarGet(VAR_GIO_CHAPTER);
    snapshot->checkpointAct = VarGet(VAR_GIO_ACT);
    snapshot->checkpointMapGroup = VarGet(VAR_GIO_CHECKPOINT_MAP_GROUP);
    snapshot->checkpointMapNum = VarGet(VAR_GIO_CHECKPOINT_MAP_NUM);
    snapshot->checkpointX = VarGet(VAR_GIO_CHECKPOINT_X);
    snapshot->checkpointY = VarGet(VAR_GIO_CHECKPOINT_Y);
    snapshot->checkpointStage = VarGet(VAR_GIO_CHECKPOINT_STAGE);
    snapshot->checkpointSaveAllowed = saveAllowedAtCheckpoint;
    snapshot->checkpointObjectiveFlags = GetGiovanniCheckpointObjectiveFlags();
}
...
static bool8 RestoreGiovanniActCheckpointStateFromSnapshot(void)
{
    ...
    if (!snapshot->valid || snapshot->checkpointChapter < 1 || snapshot->checkpointChapter > 3)
        return FALSE;

    VarSet(VAR_GIO_CHAPTER, snapshot->checkpointChapter);
    VarSet(VAR_GIO_ACT, max(1, snapshot->checkpointAct));
    VarSet(VAR_GIO_CHECKPOINT_MAP_GROUP, snapshot->checkpointMapGroup);
    VarSet(VAR_GIO_CHECKPOINT_MAP_NUM, snapshot->checkpointMapNum);
    VarSet(VAR_GIO_CHECKPOINT_X, snapshot->checkpointX);
    VarSet(VAR_GIO_CHECKPOINT_Y, snapshot->checkpointY);
    VarSet(VAR_GIO_CHECKPOINT_STAGE, snapshot->checkpointStage);
    ApplyGiovanniCheckpointObjectiveFlags(snapshot->checkpointObjectiveFlags);
    RunGiovanniMemoryModeResetHooks(snapshot->checkpointChapter);
    return TRUE;
}
...
static void SaveGiovanniCheckpointPositionAndStage(u8 chapterId, bool8 saveAllowedAtCheckpoint)
{
    ...
    SnapshotGiovanniActCheckpointState(saveAllowedAtCheckpoint);
}

static void SaveGiovanniCheckpointPositionAndStageAtMap(u8 chapterId, u8 mapGroup, u8 mapNum, u8 x, u8 y, bool8 saveAllowedAtCheckpoint)
{
    u16 chapterStageVar;
...
static void SaveGiovanniAct1StartCheckpoint(u8 chapterId, bool8 saveAllowedAtCheckpoint)
{
    ...
    {
    case 1:
        SaveGiovanniCheckpointPositionAndStageAtMap(chapterId,
                                                    MAP_GROUP(MAP_ROCKET_HIDEOUT_B4F),
                                                    MAP_NUM(MAP_ROCKET_HIDEOUT_B4F),
                                                    19,
                                                    6,
                                                    saveAllowedAtCheckpoint);
        break;
    case 2:
        SaveGiovanniCheckpointPositionAndStageAtMap(chapterId,
...
static void UpdateGiovanniCheckpointFromRocketOpsStage(u8 chapterId, u8 chapterStage)
{
    ...
    if (chapterId == 1 && chapterStage >= 2)
        checkpointId = 1;
    else if (chapterId == 3)
    {
        bool8 checkpoint1Complete = FlagGet(FLAG_GIO_MEM_CH3_ESCORT_CHECKPOINT_1) || chapterStage >= 2;
        bool8 checkpoint2Complete = chapterStage >= 3;

        if (!checkpoint2Complete && FlagGet(FLAG_GIO_MEM_CH3_ESCORT_CHECKPOINT_2))
        {
            if (FlagGet(FLAG_ROCKETOPS_ROUTE_SECURED)
             || FlagGet(FLAG_GIO_MEM_CH3_MIDPOINT_AMBUSH_CLEARED))
            {
        ...
        if (checkpoint2Complete)
            checkpointId = max(checkpointId, 2);
        else if (checkpoint1Complete)
            checkpointId = max(checkpointId, 1);
    }

    VarSet(VAR_GIO_CHECKPOINT_ID, GetSanitizedGiovanniCheckpointId(chapterId, checkpointId));
    WriteGiovanniActCheckpointFromCurrentState();
}

static bool8 RestoreGiovanniCheckpointContextForRestart(bool8 setWarp)
{
    u8 chapterId;
    u8 checkpointId;
    u16 chapterStageVar;
    ...
    if (!LoadGiovanniMemoryPartyTemplate(chapterId))
        return FALSE;
...
static void ResetGiovanniChapter3EscortSegmentState(void)
{
    FlagClear(FLAG_GIO_MEM_CH3_ESCORT_SEGMENT_ACTIVE);
    FlagClear(FLAG_GIO_MEM_CH3_ESCORT_CHECKPOINT_1);
    FlagClear(FLAG_GIO_MEM_CH3_ESCORT_CHECKPOINT_2);
...
static void ReconcileGiovanniChapter3EscortSegmentState(void)
{
    ...
    if (FlagGet(FLAG_GIO_MEM_CH3_EVAC_COMPLETE))
    {
        FlagClear(FLAG_GIO_MEM_CH3_ESCORT_SEGMENT_ACTIVE);
        return;
    }
...
static void ReconcileGiovanniEscortAndConvoyRecoveryState(u8 chapterId, u8 chapterStage, u8 checkpointId)
{
    ...
    else if (chapterId == 3)
    {
        ...
        else if (checkpointId >= 1)
        {
            ...
            FlagClear(FLAG_GIO_MEM_CH3_MIDPOINT_AMBUSH_CLEARED);
            FlagClear(FLAG_GIO_MEM_CH3_EVAC_COMPLETE);
            FlagClear(FLAG_ROCKETOPS_STAFF_EXTRACTED);
            FlagClear(FLAG_GIO_MEM_CH3_EVAC_COLLAPSE_VISUAL);
        }
...
static void ResetRocketOpsState(void)
{
    ...
    FlagClear(FLAG_GIO_MEM_CH1_CONVOY_MOVED_VISUAL);
    FlagClear(FLAG_GIO_MEM_CH1_CHECKPOINT_SECURED_VISUAL);
    FlagClear(FLAG_GIO_MEM_CH2_ALARM_ESCALATED_VISUAL);
    FlagClear(FLAG_GIO_MEM_CH2_SERVER_DESTROYED_VISUAL);
    FlagClear(FLAG_GIO_MEM_CH2_ROUTE_UNSEALED_VISUAL);
    FlagClear(FLAG_GIO_MEM_CH3_ARCHIVE_EXPLOSION_VISUAL);
    FlagClear(FLAG_GIO_MEM_CH3_POWER_OUTAGE_VISUAL);
    FlagClear(FLAG_GIO_MEM_CH3_EVAC_COLLAPSE_VISUAL);
}
...
bool8 HandleGiovanniMemoryModeWhiteout(void)
{
    ...
    RestoreGiovanniActCheckpointStateFromSnapshot();
    chapterId = GetGiovanniMemoryModeChapterId();

    if (IsGiovanniMemorySaveBlocked())
    {
        if (VarGet(VAR_GIO_ACT) <= 1)
            SaveGiovanniAct1StartCheckpoint(chapterId, FALSE);
        else
            SaveGiovanniCheckpointPositionAndStage(chapterId, FALSE);
    }
...
bool8 HandleGiovanniMemoryModeBootstrapOnLoad(void)
{
    ...
    else if (RestoreGiovanniCheckpointContextForRestart(FALSE))
    {
        ...
        {
            // Keep the saved checkpoint location for between-act retries.
        }
        else if (chapterId == 1)
        {
...
u16 Special_RocketOps_WritebackState(void)
{
    ...
    if (VarGet(chapterStageVar) > previousStage)
    {
        if (previousStage < 3 && VarGet(chapterStageVar) >= 3)
            UpdateGiovanniMemorySaveStateForSegment(GIO_SEGMENT_3, GIO_SEGMENT_1, TRUE);
...
u16 ValidateGiovanniInteractionOverlayForCurrentMap(void)
{
    ...
    overlay = GetGiovanniOverlayForMap(map, chapterId);
    if (overlay == NULL)
        return TRUE;
...
u16 Special_GiovanniCampaignDispatch(void)
{
    ...
    if (segment == GIO_SEGMENT_7)
    {
        actId++;
        VarSet(VAR_GIO_ACT, actId);
        UpdateGiovanniMemorySaveStateForSegment(GIO_SEGMENT_1, GIO_SEGMENT_3, FALSE);
        VarSet(VAR_GIO_ACT_BEAT_INDEX, 0);
        VarSet(VAR_GIO_ACT_TIMER_BUCKET, 0);
        SetGiovanniCampaignSegment(GIO_SEGMENT_1);
        RefreshGiovanniActiveDirective();
